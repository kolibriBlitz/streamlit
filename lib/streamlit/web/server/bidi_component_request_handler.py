...
class BidiComponentRequestHandler(tornado.web.RequestHandler):
    ...
    def get(self, path: str) -> None:
        ...
        component_def = self._registry.get(component_name)
        if component_def is None:
            self.write("not found")
            self.set_status(404)
            return

        # HTML files should not be served directly since HTML is only accepted as a string
        if len(parts) > 1 and parts[1].endswith(".html"):
            self.write("not found")
            self.set_status(404)
            return

        # Get the component path from the registry
